...
### New Controls:
+   SV-270174 - RHEL 9 must display the Standard Mandatory DOD Notice and Consent Banner before granting local or remote access to the system via a graphical user logon.
+   SV-270175 - RHEL 9 &quot;&#x2F;etc&#x2F;audit&#x2F;&quot; must be owned by root.
+   SV-270176 - RHEL 9 &quot;&#x2F;etc&#x2F;audit&#x2F;&quot; must be group-owned by root.
+   SV-270177 - The RHEL 9 SSH client must be configured to use only DOD-approved encryption ciphers employing FIPS 140-3 validated cryptographic hash algorithms to protect the confidentiality of SSH client connections.
+   SV-270178 - The RHEL 9 SSH client must be configured to use only DOD-approved Message Authentication Codes (MACs) employing FIPS 140-3 validated cryptographic hash algorithms to protect the confidentiality of SSH client connections.
+   SV-270180 - The RHEL 9 fapolicy module must be configured to employ a deny-all, permit-by-exception policy to allow the execution of authorized software programs.
+   SV-272488 - RHEL 9 must have the Postfix package installed.
+   SV-272496 - RHEL 9 must elevate the SELinux context when an administrator calls the sudo command.
...
Old: 
```
Verify the system-wide shared library directories are group-owned
...
Old: 
```
Verify RHEL 9 remote access using SSH prevents logging on with a blank password with the following command:
...
Old: 
```
Verify that RHEL 9 audit service package is installed.
...
Old: 
```
Configure the RHEL 9 SSH client to use only MACs employing FIPS 140-3 approved algorithms by updating the "/etc/crypto-policies/back-ends/openssh.config" file with the following line:
...
<details>
  <summary>Click to expand.</summary>
SV-257794:
Old: RHEL 9 must clear SLUB&#x2F;SLAB objects to prevent use-after-free attacks.
New: RHEL 9 must clear memory when it is freed to prevent use-after-free attacks.
---
SV-257888:
Old: RHEL 9 cron configuration directories must have a mode of 0700 or less permissive.
New: RHEL 9 permissions of cron configuration files and directories must not be modified from the operating system defaults.
---
SV-257949:
Old: RHEL 9 must configure a DNS processing mode set be Network Manager.
New: RHEL 9 must configure a DNS processing mode in Network Manager.
---
SV-257988:
Old: RHEL 9 must implement DOD-approved encryption ciphers to protect the confidentiality of SSH client connections.
New: RHEL 9 must implement DOD-approved encryption ciphers to protect the confidentiality of SSH connections.
---
SV-257989:
Old: RHEL 9 must implement DOD-approved encryption ciphers to protect the confidentiality of SSH server connections.
New: The RHEL 9 SSH server must be configured to use only DOD-approved encryption ciphers employing FIPS 140-3 validated cryptographic hash algorithms to protect the confidentiality of SSH server connections.
---
SV-257991:
Old: RHEL 9 SSH server must be configured to use only Message Authentication Codes (MACs) employing FIPS 140-3 validated cryptographic hash algorithms.
New: The RHEL 9 SSH server must be configured to use only Message Authentication Codes (MACs) employing FIPS 140-3 validated cryptographic hash algorithms to protect the confidentiality of SSH server connections.
---
SV-257998:
Old: RHEL 9 SSH server configuration file must be owned by root.
New: The RHEL 9 SSH server configuration file must be owned by root.
---
SV-257999:
Old: RHEL 9 SSH server configuration file must have mode 0600 or less permissive.
New: RHEL 9 SSH server configuration files&#39; permissions must not be modified.
---
SV-258068:
Old: RHEL 9 must automatically exit interactive command shell user sessions after 15 minutes of inactivity.
New: RHEL 9 must automatically exit interactive command shell user sessions after 10 minutes of inactivity.
---
SV-258121:
Old: RHEL 9 must use the CAC smart card driver.
New: RHEL 9 must use the common access card (CAC) smart card driver.
---
SV-258232:
Old: RHEL 9 IP tunnels must use FIPS 140-2&#x2F;140-3 approved cryptographic algorithms.
New: RHEL 9 IP tunnels must use FIPS 140-3 approved cryptographic algorithms.
---
SV-258236:
Old: RHEL 9 crypto policy must not be overridden.
New: RHEL 9 cryptographic policy must not be overridden.
---
SV-258241:
Old: RHEL 9 must implement a system-wide encryption policy.
New: RHEL 9 must implement a FIPS 140-3 compliant systemwide cryptographic policy.
---
</details>
